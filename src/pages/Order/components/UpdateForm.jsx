...
import {Form, Input, Modal, Select, Upload, Icon, message, InputNumber, Drawer, Row, Col, Button} from 'antd';
...
const CreateForm = props => {
  ...
  return (
    ...
    >
      ...
      <div>支付状态：{values.payStatus == 0 ? '已支付' : values.payStatus == 1 ? '未支付' : values.payStatus == 2 ? '已退款' : '已取消'}</div>
      ...
      {/*productType: "啤酒"*/}
      {/*productTypeId: 2*/}
      {values.orderGoodsList && values.orderGoodsList.map(value => {
        return <Row>
      ...
      <div>
        ...
        <Select defaultValue={status} style={{width: 150}} onChange={changeStatus}>
          {values.status == 0 && <Option value='0' disable>等待商家确认</Option>}
        ...
        <Button style={{marginLeft:10}} type="primary" onClick={okHandle}>修改状态</Button>
      </div>
    </Drawer>
  );
};
